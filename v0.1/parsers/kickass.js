...
function KickAss() {
	...
	this.search = function(title,cb) {
			http.get(pbsearch+escape(title), function(res) {
					...
					res.on('end', function(){
			var results = getSplit(body,"mainSearchTable\">",1);if(results == null){console.log("Parse error (kickasstorrents.to)");cb({site:"kickasstorrents.to",results:null});return;}
							results = getSplit(results,"</tbody>",0);if(results == null){console.log("Parse error (kickasstorrents.to)");cb({site:"kickasstorrents.to",results:null});return;}
			results = results.split("id=\"torrent_");
							var res = [];
							for(i=1;i<results.length;i++) {
									var tmp = new torrent();
									tmp.title = results[i].substring(results[i].search(/cellMainLink">/)+14,results[i].indexOf("</",results[i].search(/cellMainLink"/))).replace(/\./g," ");
